...
\begin{abstract}
La Arquitectura de Confianza Cero (\textit{Zero Trust Architecture}, ZTA) representa
...
\textbf{Diseño de software bajo la premisa de red comprometida: autenticación y autorización granular en microservicios}



\begin{mdframed}[style=quoteStyle]
\textit{\textbf{Área temática:} Ciberseguridad · Arquitectura de Software · DevSecOps}
\textit{\textbf{Institución:} Universidad Nacional del Nordeste — Ingeniería del SW 2}
\textit{\textbf{Palabras clave:} Zero Trust Architecture (ZTA), microservicios, mTLS, SPIFFE/SPIRE, OAuth 2.0, OIDC, OPA, service mesh, microsegmentación, autenticación continua}
\end{mdframed}
...
\begin{itemize}
  \item 3.1. Principios axiales
  \item 3.2. El modelo NIST SP 800-207 y SP 800-207A
  \item 3.3. Componentes lógicos: PEP, PDP, PA
  \item 5.1. Identidad como nuevo perímetro
  \item 5.2. Identidad de carga de trabajo: SPIFFE y SPIRE
  \item 5.3. Autenticación mutua: mTLS
  \item 5.4. Autenticación de usuarios: OAuth 2.0 y OpenID Connect
  \item 5.5. Autorización granular: Open Policy Agent (OPA)
  \item 5.6. Service Mesh: Istio y Envoy como plano de control de seguridad
\end{itemize}
...
\section{2. Contexto Histórico y Evolución del Paradigma}

La genealogía de Zero Trust puede trazarse a través de varias etapas intelectuales y técnicas:

\textbf{1994 — Primeras formulaciones conceptuales.} Stephen Paul Marsh introduce la noción formal de confianza computacional en su tesis doctoral, estableciendo bases filosóficas que anticipan el modelo (Marsh, 1994, citado en Nasiruzzaman et al., 2024).

\textbf{2003 — Jericho Forum y la des-perimeterización.} El consorcio Jericho Forum publica los primeros lineamientos sobre \textit{de-perimeterización}, argumentando que la frontera entre una organización y el mundo exterior se estaba volviendo irrelevante. El foro se disuelve en 2013, declarando que la des-perimeterización ya es un hecho consumado (Zscaler, 2025).

\textbf{2009–2010 — Kindervag y el término "Zero Trust".} John Kindervag, analista de Forrester Research, acuña formalmente el término \textit{Zero Trust} en su informe seminal \textit{"No More Chewy Centers: Introducing The Zero Trust Model of Information Security"} (2010). Su propuesta fue radical: la confianza debía eliminarse como categoría de diseño de seguridad. El argumento central es que los profesionales de seguridad confían demasiado y verifican muy poco (1Password, 2024). Kindervag propuso tres principios fundacionales: (a) asegurar el acceso a todos los recursos independientemente de su ubicación, (b) adoptar el mínimo privilegio y controlar estrictamente el acceso, y (c) inspeccionar y registrar todo el tráfico.

\textbf{2009–2014 — Google BeyondCorp.} En 2009, en respuesta al ciberataque conocido como \textit{Operación Aurora}, Google inicia internamente el proyecto BeyondCorp con el objetivo de permitir el trabajo remoto sin VPN, basando el acceso en la identidad y el contexto del dispositivo, no en la ubicación de red. La publicación del modelo en 2014 otorgó al concepto de Zero Trust una validación de escala industrial (Zscaler, 2025).

\textbf{2017–2019 — Expansión y marcos complementarios.} Gartner introduce CARTA (\textit{Continuous Adaptive Risk and Trust Assessment}, 2017) y SASE (\textit{Secure Access Service Edge}, 2019), marcos que comparten la filosofía de verificación adaptativa y continua con ZTA.

\textbf{2020 — NIST SP 800-207.} El Instituto Nacional de Estándares y Tecnología de los Estados Unidos publica la publicación especial 800-207, convirtiéndola en el marco de referencia normativo más completo para ZTA a nivel global. En 2023, NIST publica SP 800-207A, extendiendo la guía específicamente hacia aplicaciones cloud-nativas en entornos multi-nube.

\textbf{2021 — Mandato ejecutivo (EO 14028).} La Orden Ejecutiva 14028 del gobierno de los Estados Unidos exige que todas las agencias federales adopten los principios de confianza cero para 2024, elevando ZTA de recomendación técnica a política de Estado.

\textbf{2025 — Normalización ETSI.} En septiembre de 2025, el Comité Técnico CYBER del Instituto Europeo de Normas de Telecomunicaciones (ETSI) publica la Especificación Técnica TS 104 102, que formaliza la metodología ZT-Kipling como estándar europeo (Wikipedia, 2025).



\section{3. Fundamentos Conceptuales de Zero Trust}

\subsection{3.1. Principios Axiales}
...
\textbf{Nunca confiar, siempre verificar (\textit{Never Trust, Always Verify}).} Toda entidad —usuario, servicio, dispositivo— es tratada como potencialmente hostil hasta que demuestre su identidad y su autorización para el recurso solicitado. La ubicación de red (interna o externa) no otorga confianza implícita alguna.

\textbf{Mínimo privilegio (\textit{Least Privilege}).} Cada identidad —humana o no humana— recibe únicamente los permisos estrictamente necesarios para cumplir su función, durante el tiempo mínimo requerido. Los privilegios son acotados a la sesión y se elevan solo cuando existe justificación explícita.

\textbf{Asumir la brecha (\textit{Assume Breach}).} Los sistemas deben ser diseñados bajo la suposición de que el atacante ya ha obtenido acceso a algún componente. El objetivo no es prevenir toda intrusión, sino minimizar el radio de daño (\textit{blast radius}) y detectar anomalías lo antes posible.

\textbf{Verificación explícita y continua.} La autenticación no es un evento único al inicio de la sesión, sino un proceso continuo que evalúa contexto, comportamiento e integridad de la identidad a lo largo del tiempo.

\textbf{Acceso por sesión con política dinámica.} El acceso se otorga por solicitud individual, no por pertenencia a una red. La política de acceso considera factores contextuales como el nivel de aseguramiento de identidad, la postura de seguridad del dispositivo y el comportamiento reciente de la entidad.

\textbf{Tratar todos los recursos como externos.} Independientemente de si un servicio se encuentra dentro o fuera de la red corporativa, debe ser tratado con el mismo nivel de desconfianza y controles de seguridad.

\textbf{Gobernar todas las identidades consistentemente.} Las políticas de acceso aplican de forma uniforme sobre identidades humanas y no humanas (servicios, pipelines CI/CD, agentes de software).

\subsection{3.2. El Modelo NIST SP 800-207 y SP 800-207A}
...
\subsection{3.3. Componentes Lógicos: PEP, PDP y PA}

El modelo NIST articula la ZTA alrededor de tres componentes lógicos principales:

\textbf{Policy Enforcement Point (PEP) — Punto de Ejecución de Políticas.} Es el guardián que intercepta cada solicitud de acceso y la evalúa contra las políticas vigentes. En arquitecturas de microservicios, el proxy Envoy (parte de Istio y otros service meshes) actúa como PEP universal al interceptar todo el tráfico entrante y saliente de cada servicio (Tetrate, 2023).

\textbf{Policy Decision Point (PDP) — Punto de Decisión de Políticas.} Es el componente que evalúa la solicitud de acceso según las políticas definidas y emite una decisión de permitir o denegar. Un ejemplo típico es Open Policy Agent (OPA) actuando como motor de decisión externo.

\textbf{Policy Administrator (PA) — Administrador de Políticas.} Gestiona el ciclo de vida de las políticas, incluyendo su creación, actualización, distribución y revocación. Se encarga de comunicar las decisiones del PDP al PEP.
...
\section{4. El Problema de la Seguridad Perimetral en Arquitecturas Distribuidas}
...
\section{5. Zero Trust Aplicado al Desarrollo de Microservicios}

\subsection{5.1. Identidad como Nuevo Perímetro}
...
La \textbf{identidad humana} corresponde a los usuarios finales o administradores que interactúan con el sistema. Se gestiona mediante protocolos estándar como OAuth 2.0 y OpenID Connect (OIDC).

La \textbf{identidad de carga de trabajo} (\textit{workload identity}) corresponde a los propios microservicios, funciones serverless, jobs de CI/CD y agentes de software que se comunican entre sí. Esta dimensión es frecuentemente subestimada pero resulta crítica en arquitecturas distribuidas.

\subsection{5.2. Identidad de Carga de Trabajo: SPIFFE y SPIRE}

El estándar SPIFFE (\textit{Secure Production Identity Framework For Everyone}) define una identidad verificable para software en entornos dinámicos y heterogéneos. SPIRE es su implementación de referencia, gestionada por la Cloud Native Computing Foundation (CNCF).
...
Cada carga de trabajo, al iniciarse, demuestra su naturaleza e identidad mediante un proceso de atestación (\textit{attestation}). SPIRE verifica atributos como el nombre del pod en Kubernetes, la imagen de contenedor o la función de nube, y emite un documento de identidad de corta duración denominado SVID (\textit{SPIFFE Verifiable Identity Document}). Los SVIDs son certificados X.509 con un tiempo de vida típico de una hora, que se renuevan automáticamente, eliminando la necesidad de credenciales estáticas (Sameerbhanushali, 2026).
...
Como destacan los investigadores del paper \textit{"Zero Trust Security Model Implementation in Microservices Architectures Using Identity Federation"} (arXiv, 2511.04925, 2025), SPIFFE/SPIRE permite construir identidades de extremo a extremo que facilitan conexiones mTLS seguras e identidades de carga de trabajo auditables en entornos multi-dominio.

\subsection{5.3. Autenticación Mutua: mTLS}

El \textit{Mutual TLS} (mTLS) es el mecanismo fundamental para la autenticación entre servicios en una arquitectura Zero Trust. A diferencia del TLS estándar —donde solo el servidor se autentica ante el cliente— en mTLS ambas partes deben presentar certificados válidos para establecer la comunicación.

En el contexto de microservicios, mTLS garantiza que:

\begin{itemize}
  \item Cada solicitud entre servicios está cifrada en tránsito, incluyendo el tráfico este-oeste dentro del clúster.
  \item Cada servicio puede verificar criptográficamente la identidad del servicio que lo invoca.
  \item Los certificados son gestionados automáticamente por el service mesh, sin intervención manual.
\end{itemize}
...
\subsection{5.4. Autenticación de Usuarios: OAuth 2.0 y OpenID Connect}
...
\textbf{OAuth 2.0} es un framework de autorización que permite a los clientes obtener acceso limitado a recursos en nombre de un usuario, sin exponer sus credenciales. En arquitecturas de microservicios, los tokens de acceso (generalmente JWT) son emitidos por un servidor de autorización central (\textit{Identity Provider}) y presentados en cada solicitud a los servicios.

\textbf{OpenID Connect (OIDC)} extiende OAuth 2.0 añadiendo una capa de identidad, definiendo el mecanismo de SSO (\textit{Single Sign-On}) y de acceso basado en \textit{claims} a través de API gateways o patrones Backend-for-Frontend (BFF). OIDC Federation 1.0 extiende este modelo permitiendo la federación automatizada y segura de múltiples dominios de confianza mediante declaraciones de entidad firmadas (arXiv, 2511.04925, 2025).
...
\subsection{5.5. Autorización Granular: Open Policy Agent (OPA)}

Una vez establecida la autenticación, el siguiente desafío es la autorización granular. OPA (\textit{Open Policy Agent}) es un motor de políticas de código abierto, de propósito general, que desacopla la toma de decisiones de autorización del código de aplicación.

OPA permite definir políticas como código usando el lenguaje declarativo Rego, y evaluarlas de forma centralizada o distribuida. En el contexto de microservicios con Istio, los proxies Envoy utilizan el filtro de \textit{External Authorization} para delegar cada solicitud al motor OPA antes de permitir que alcance el servicio destino (AWS, 2025; OPA Docs, 2025).

Las ventajas de este enfoque son múltiples:

\begin{itemize}
  \item Las políticas de autorización son externas al código del servicio, garantizando consistencia entre todos los microservicios.
  \item Las políticas pueden actualizarse sin redesplegar los servicios.
  \item Las políticas son versionables, testeables y auditables como código fuente.
  \item OPA puede integrarse en sidecars para evaluar políticas localmente, evitando latencia de red adicional (Permit.io, 2025).
\end{itemize}
...
Un sistema de autorización contemporáneo en microservicios debe responder preguntas compuestas del tipo: \textit{"¿Está el Servicio A autorizado a acceder al Recurso X en nombre del Usuario Y?"} Esta complejidad requiere un sistema de autorización desacoplado como OPA, en lugar de lógica de autorización embebida en cada servicio (Cerbos, 2025).

\subsection{5.6. Service Mesh: Istio y Envoy como Plano de Control de Seguridad}
...
\textbf{Istio} es el service mesh más ampliamente adoptado en entornos Kubernetes. Su proxy de datos —Envoy— se despliega como \textit{sidecar} junto a cada microservicio, interceptando todo el tráfico entrante y saliente. Este patrón convierte a Envoy en un PEP universal que puede:

\begin{itemize}
  \item Implementar mTLS automáticamente entre todos los servicios del mesh.
  \item Validar SVIDs de SPIFFE como identidades de servicio.
  \item Delegar decisiones de autorización a OPA mediante el filtro de External Authorization.
  \item Generar telemetría detallada de todo el tráfico para observabilidad y auditoría.
\end{itemize}
...
\section{6. Microsegmentación y Control de Movimiento Lateral}
...
\section{7. Gestión de Secretos y Credenciales Efímeras}
...
Zero Trust aborda este problema mediante el principio de credenciales efímeras (\textit{short-lived credentials}): en lugar de distribuir secretos de larga duración, los servicios obtienen credenciales de corta vida a través de mecanismos autenticados.

Las plataformas de gestión de secretos más adoptadas en este paradigma incluyen:

\textbf{HashiCorp Vault} es la solución más difundida. Permite a los servicios autenticarse ante Vault utilizando su SVID de SPIFFE como credencial de entrada, y obtener a cambio secretos dinámicos —como credenciales de base de datos generadas en tiempo real con TTL corto. El resultado es un flujo donde no existen secretos estáticos que distribuir ni rotar manualmente: Vault genera credenciales dinámicas por solicitud y las revoca automáticamente al expirar (Sameerbhanushali, 2026).

\textbf{AWS Secrets Manager, GCP Secret Manager, Azure Key Vault} son equivalentes gestionados por los principales proveedores de nube, con integración nativa con sus respectivos modelos de identidad de servicio.
...
\section{8. Integración con DevSecOps y la Cadena de Suministro de Software}

Zero Trust no es únicamente un paradigma de infraestructura en tiempo de ejecución (\textit{runtime}); debe integrarse en el ciclo de vida completo del desarrollo de software, desde la escritura de código hasta el despliegue en producción.

Este enfoque —frecuentemente denominado \textit{DevSecOps} con principios Zero Trust— implica los siguientes controles en cada etapa de la cadena de entrega:

\textbf{Fase de desarrollo:} Escaneo estático de código (SAST) para detectar secretos hardcodeados, vulnerabilidades conocidas y mal uso de APIs de seguridad. Commits firmados criptográficamente para garantizar la integridad del código fuente.
...
\textbf{Fase de despliegue (CD):} Implementación de admission controllers en Kubernetes que verifiquen la firma de imágenes y la conformidad con políticas de seguridad antes de permitir que un pod se inicie. Restricción de privilegios de despliegue al mínimo necesario.

\textbf{Fase de runtime:} Enforcement de Network Policies, mTLS, validación de JWT y políticas OPA en cada solicitud. Rotación automática de secretos y certificados.
...
\section{9. Monitoreo Continuo, Observabilidad y Telemetría}

El principio de \textit{verificación continua} de Zero Trust requiere que el sistema no solo autentique y autorice en el momento del acceso, sino que monitoree de forma permanente el comportamiento de todas las entidades para detectar anomalías o comportamientos sospechosos.

En una arquitectura ZTA madura, el componente de observabilidad incluye:

\textbf{Telemetría del service mesh.} El proxy Envoy en Istio genera métricas, logs distribuidos y trazas de cada solicitud inter-servicio. Esta telemetría permite mapear cada interacción entre servicios —condición previa fundamental: no se puede asegurar lo que no se puede ver (Developers.dev, 2026).

\textbf{User and Entity Behavior Analytics (UEBA).} Plataformas de análisis de comportamiento que establecen líneas base de acceso normal para cada entidad y alertan ante desviaciones estadísticas significativas. Un servicio que normalmente accede solo a la base de pagos y de repente consulta la base de usuarios representa una anomalía que debe investigarse.

\textbf{SIEM (Security Information and Event Management).} Centralización y correlación de logs de múltiples fuentes (identity provider, API gateway, service mesh, Kubernetes audit logs) para detectar patrones de ataque que no serían visibles en ninguna fuente individual.

\textbf{Telemetría de identidad.} Los proveedores de identidad (como Keycloak, Okta o Azure AD) generan logs de autenticación que, correlacionados con la telemetría de red, permiten detectar uso anómalo de credenciales (logins desde ubicaciones inusuales, rotación anormal de tokens, escalaciones de privilegios no esperadas).
...
\section{10. Desafíos e Implicancias Operativas}
...
\subsection{Overhead de rendimiento y latencia}
...
\subsection{Costos de migración y operativos}
...
\subsection{Configuración errónea como vector de riesgo}

Paradójicamente, una implementación incorrecta de ZTA puede introducir nuevos riesgos. Políticas mal configuradas pueden bloquear comunicaciones legítimas en producción o, en el caso opuesto, dejar brechas no cubiertas. La filosofía de \textit{deny all by default} requiere un mapeo exhaustivo previo de todas las comunicaciones legítimas entre servicios.

\subsection{Brechas en auditoría y cumplimiento}
...
\subsection{Fricción organizacional}
...
\section{11. Casos de Referencia y Adopción Industrial}

\textbf{Google BeyondCorp.} El caso más documentado de implementación Zero Trust a escala. Google eliminó la dependencia de la VPN corporativa y basó todo el acceso —tanto interno como externo— en la identidad verificada del usuario y la postura del dispositivo. Este modelo permitió a todos los empleados trabajar de forma segura desde cualquier red, sin distinción entre interna y externa.

\textbf{Microsoft Zero Trust.} Microsoft implementó ZTA internamente y lo formalizó en su \textit{Zero Trust Deployment Center}, que incluye guías detalladas para identidades, dispositivos, aplicaciones, datos, infraestructura y redes. La documentación de Sunkara (2025) compara las implementaciones prácticas de Google y Microsoft como referencias industriales.

\textbf{Sector financiero y DevSecOps cloud-native.} Estudios publicados en ScienceDirect (2025) analizan cómo el sector financiero adopta ZTA en entornos DevOps avanzados con microservicios, combinando CI/CD continuo con verificación de seguridad en cada etapa del pipeline para cumplir con regulaciones del sector.

\textbf{Amazon Web Services (AWS) y Kubernetes.} El blog de código abierto de AWS documenta implementaciones concretas de Zero Trust en Amazon EKS (Elastic Kubernetes Service) usando Istio con OPA como servicio de autorización externo, como referencia de implementación reproducible (AWS, 2025).



\section{12. Tendencias Emergentes: IA, Multi-cloud y Agentes de Software}
...
\textbf{Criptografía post-cuántica.} Con el avance de la computación cuántica, la resistencia de los algoritmos criptográficos actuales (RSA, ECDSA) utilizados en mTLS y PKI está siendo evaluada. NIST ha iniciado el proceso de estandarización de algoritmos resistentes a quantum, lo que eventualmente requerirá actualizaciones en las infraestructuras de SPIFFE/SPIRE y certificados TLS.
...
\begin{mdframed}[style=quoteStyle]
\textit{\textit{Las referencias se presentan en orden alfabético. Se incluyen fuentes académicas, normativas e industriales de referencia.}}
\end{mdframed}

AccuKnox. (2025). \textit{Best Microsegmentation Tools For Zero Trust Security 2025}. AccuKnox Blog. https://accuknox.com/blog/microsegmentation-tools-zero-trust

AccuKnox. (2026). \textit{How To Implement Zero Trust Architecture (Reduce Security Risks)}. AccuKnox Blog. https://accuknox.com/blog/zero-trust-architechture

Amazon Web Services. (2025). \textit{Achieving Zero Trust Security on Amazon EKS with Istio}. AWS Open Source Blog. https://aws.amazon.com/blogs/opensource/achieving-zero-trust-security-on-amazon-eks-with-istio/

Cerbos. (2025). \textit{Zero-Trust for Microservices, a Practical Blueprint}. Cerbos Blog. https://www.cerbos.dev/blog/zero-trust-for-microservices

Cloudaware. (2026). \textit{Implementing Zero Trust in DevSecOps Workflow in 2026}. Cloudaware Blog. https://cloudaware.com/blog/zero-trust-devsecops/

DEV Community. (2026). \textit{Authorization methods in .NET microservices}. https://dev.to/belochka1-04/authorization-methods-in-net-microservices-1dl0

Developers.dev. (2026). \textit{Zero Trust Microservices: mTLS \& Identity Architecture Guide}. https://www.developers.dev/tech-talk/the-architect-s-decision-implementing-zero-trust-security-in-microservices-at-scale.html

DZone. (2025). \textit{Building Bulletproof Infrastructure With Istio and OPA}. https://dzone.com/articles/zero-trust-api-security-istio-opa

Groundcover. (2026). \textit{Zero Trust in Kubernetes: Principles, Architecture \& Best Practices}. https://www.groundcover.com/learn/security/zero-trust-kubernetes

Hofmann, M. (2025). \textit{Zero Trust with Microservices - it's easier than you think!} JAX London 2025. https://jaxlondon.com/blog/zero-trust-with-microservices-its-easier-than-you-think/

IBM. (2025). \textit{Cost of a Data Breach Report 2025}. IBM Security.

Kesarpu, S. (2025). \textit{Zero-Trust Architecture in Java Microservices}. International Journal of Networks and Security. https://www.academicpublishers.org/journals/index.php/ijns/article/view/5451

Kindervag, J. (2010). \textit{No More Chewy Centers: Introducing The Zero Trust Model Of Information Security}. Forrester Research.

Markaicode. (2025). \textit{Zero-Trust APIs in 2025: OAuth 3.0 and Open Policy Agent (OPA) Deep Dive}. https://markaicode.com/zero-trust-apis-oauth-opa-guide/

Nasiruzzaman, M., Ali, M., Salam, I., \& Hossain, M. H. (2024). \textit{The Evolution of Zero Trust Architecture (ZTA) from Concept to Implementation}. arXiv preprint arXiv:2504.11984. https://arxiv.org/abs/2504.11984

National Institute of Standards and Technology. (2020). \textit{Zero Trust Architecture} (NIST Special Publication 800-207). U.S. Department of Commerce. https://doi.org/10.6028/NIST.SP.800-207

National Institute of Standards and Technology. (2023). \textit{A Zero Trust Architecture Model for Access Control in Cloud-Native Applications in Multi-Cloud Environments} (NIST Special Publication 800-207A). U.S. Department of Commerce. https://csrc.nist.gov/pubs/sp/800/207/a/final

Northern Technologies Group. (2025). \textit{Zero Trust Architecture in 2025: Shifting from Perimeter Security to Never Trust, Always Verify}. https://ntgit.com/zero-trust-architecture-in-2025-shifting-from-perimeter-security-to-never-trust-always-verify/

Open Policy Agent. (2025). \textit{OPA-Envoy Plugin}. OPA Documentation. https://www.openpolicyagent.org/docs/envoy

Open Policy Agent. (2025). \textit{Tutorial: Istio}. OPA Documentation. https://www.openpolicyagent.org/docs/envoy/tutorial-istio

Palo Alto Networks. (2026). \textit{What Is NIST SP 800-207? Zero Trust Architecture Framework}. Cyberpedia. https://www.paloaltonetworks.com/cyberpedia/what-is-nist-sp-800-207

Permit.io. (2025). \textit{Authorization with Open Policy Agent (OPA)}. https://www.permit.io/blog/authorization-with-open-policy-agent-opa

PMC — PubMed Central. (2025). \textit{A Systematic Literature Review on the Implementation and Challenges of Zero Trust Architecture Across Domains}. Sensors 2025, 25, 6118. https://pmc.ncbi.nlm.nih.gov/articles/PMC12526847/

Researchgate — Adanigbo et al. (2024). \textit{Implementing Zero Trust Security in Multi-Cloud Microservices Platforms: A Review and Architectural Framework}. International Journal of Advanced Multidisciplinary Research Studies, 4(6), 2402–2409.

Sameerbhanushali. (2026). \textit{SPIFFE \& SPIRE: The Workload Identity Standard Quietly Powering Zero Trust}. Substack. https://sameerbhanushali.substack.com/p/spiffe-and-spire-the-workload-identity

Seraphic Security. (2026). \textit{Adopting Zero Trust in 2025: A Practical Guide}. https://seraphicsecurity.com/learn/zero-trust/adopting-zero-trust-in-2025-a-practical-guide/

ScienceDirect — Elsevier. (2025). \textit{Enhancing cloud-native DevSecOps: A Zero Trust approach for the financial sector}. https://www.sciencedirect.com/science/article/abs/pii/S0920548925000042

Techspective. (2025). \textit{Breach Ready: Rethinking Zero Trust and Lateral Movement Defense}. https://techspective.net/2025/04/22/breach-ready-rethinking-zero-trust-lateral-movement-defense/

Tetrate. (2023). \textit{Understanding Istio and Open Policy Agent (OPA)}. Tetrate Blog. https://tetrate.io/blog/understanding-istio-and-open-policy-agent-opa

Tetrate. (2023). \textit{NIST SP 800-207A Explained: A Zero Trust Architecture Model for Access Control in Cloud-Native Applications in Multi-Cloud Environments}. https://tetrate.io/blog/nist-sp-800-207a-explained-zero-trust-architecture-model-for-access-control

The Backend Developers. (2026). \textit{Zero-Trust Service-to-Service Auth in 2026: mTLS, SPIFFE, and Identity Boundaries}. https://thebackenddevelopers.substack.com/p/zero-trust-service-to-service-auth

Wikipedia. (2025). \textit{Zero Trust Architecture}. Wikipedia. https://en.wikipedia.org/wiki/Zero\textit{trust}architecture

Wiley Online Library. (2025). \textit{Zero Trust Architecture as a Risk Countermeasure in Small–Medium Enterprises and Advanced Technology Systems}. Risk Analysis. https://doi.org/10.1111/risa.70026

Wiz. (2025). \textit{What Is Zero Trust Architecture? A Complete Guide For Cloud Security}. Wiz Academy. https://www.wiz.io/academy/compliance/zero-trust-architecture

Zscaler. (2025). \textit{A brief(er) history of zero trust: Major milestones in rethinking enterprise security}. CXO Revolutionaries. https://www.zscaler.com/blogs/product-insights/brief-er-history-zero-trust-major-milestones-rethinking-enterprise-security

arXiv. (2025). \textit{Zero Trust Security Model Implementation in Microservices Architectures Using Identity Federation}. arXiv:2511.04925. https://arxiv.org/abs/2511.04925

arXiv. (2025). \textit{Identity Control Plane: The Unifying Layer for Zero Trust Infrastructure}. arXiv:2504.17759. https://arxiv.org/pdf/2504.17759



\textit{Documento generado con fines académicos. Última actualización: junio 2026.}
